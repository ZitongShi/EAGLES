...
class OGBNDataset(object):
    ...
    def __init__(self, dataset_name='ogbn-products'):
        self.dataset_name = dataset_name
        self.dataset = PygNodePropPredDataset(name=self.dataset_name, transform=T.Compose([T.AddSelfLoops("edge_attr","mean")]))
    ...
    def generate_sub_graphs(data, parts, cluster_number=10, batch_size=1):
        ...
        for cluster in range(no_of_batches):
            ...
            sg_edges_index[cluster] = [data.edge_index_dict.get((edge[0], edge[1]), -1) for edge in
