...
class MaskedLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=True, spar_wei=False, device=None, dtype=None):
        ...
        self.threshold = nn.Parameter(torch.empty(out_features))  # 可学习的阈值参数
        ...
        self.fixed_mask = False  # 新增标志，用于控制是否固定 mask
    ...
    def forward(self, input, weight_mask=None):
        ...
        threshold = self.threshold.view(-1, 1)  # 调整阈值形状以匹配权重
        mask = self.step(abs_weight - threshold)  # 生成掩码
        assert isinstance(mask, torch.Tensor), "mask should be a Tensor"
        self.mask = mask  # 保存当前掩码
        #pdb.set_trace()
