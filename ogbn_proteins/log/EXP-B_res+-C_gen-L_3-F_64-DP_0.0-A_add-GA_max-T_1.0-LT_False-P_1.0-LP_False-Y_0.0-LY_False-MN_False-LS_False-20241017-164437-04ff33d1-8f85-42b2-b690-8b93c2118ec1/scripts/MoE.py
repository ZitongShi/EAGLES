...
class MoE(nn.Module):
    ...
    def forward(self, x, edge_index, temp, edge_attr=None, training=None):
        """
        Args:
            x: tensor shape [num_nodes, input_size]
            edge_index: tensor shape [2, num_edges]
            temp: additional tensor
            shape: additional argument
            edge_attr: tensor shape [num_edges, edge_attr_size] or None
        ...
        for i in range(self.num_experts):
            expert_i_output = self.experts[i](x, edge_index, edge_attr, temp, training)  # size:(num_edge)
        ...
        # gated_output 的计算
        gated_output = edge_gates * expert_outputs  # [num_edges, num_experts]
        ...
        k_edges_per_node = k_per_node[node_idx] * num_edges_per_node  # [num_nodes] * [num_nodes] -> [num_nodes]

        # 确保 k_edges_per_node 是正整数
        ...
        mask = mask.index_select(dim=-1, index=val_sort_idx)  # 注意这里应为 val_sort_idx 而不是 val_resort_idx

        # 稀疏后标准化（可选，根据您的需求决定是否启用）
