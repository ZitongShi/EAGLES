...
from ogbn_proteins.helper.helperfunc import fed_avg, my_fedaggregation
...
def load_masks_into_model(model, masks, fixed=True):
    """
    The loaded masks are assigned to the MaskedLinear layer in the model and the masks are fixed as needed.
    Args:
    model (torch.nn.Module): The model
    masks (list of torch.Tensor): A list of masks corresponding to each MaskedLinear layer.
    fixed (bool) -Whether to fix the mask.
    """
